...
public class MyTests {
    ...
    //@WithMockUser是創建一個假的user進行SpringSecurity的測試
    //這個假的User的帳號是mock，擁有Role是NORMAL_MEMBER
    //比較推薦根據資料庫的資料測試，因為@WithMockUser是創建假的user
    ...
    @WithMockUser(username = "mock2",roles = {"ADMIN"})
    @Test
    public void testWelcome4() throws Exception {
        RequestBuilder requestBuilder = MockMvcRequestBuilders
                .get("/welcome");
        mockMvc.perform(requestBuilder)
                .andExpect(status().is(200));
